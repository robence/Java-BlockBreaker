...
public class BlockBreakerPanel extends JPanel implements KeyListener{
	...
	BlockBreakerPanel(){
		...
		for(int i = 0; i < 8; i++){
			blocks.add(new Block((i*60+2), 50, 60, 25, "lime.png"));
		}
		for(int i = 0; i < 8; i++){
			blocks.add(new Block((i*60+2), 75, 60, 25, "brown.png"));
		}
		for(int i = 0; i < 8; i++){
			blocks.add(new Block((i*60+2), 100, 60, 25, "red.png"));
		}
		...
		blocks.get(random.nextInt(32)).powerup = true;
		blocks.get(random.nextInt(32)).powerup = true;
		ball.add(new Block(237, 437, 30, 30, "ball.png"));
		addKeyListener(this);
		setFocusable(true);
	}
	...
	public void update(){
		for(Block p : powerup){
			...
			if(p.intersects(paddle) && !p.destroyed){
				p.destroyed = true;
				ball.add(new Block(paddle.dx+75, 437, 30, 30, "ball.png"));
			}
		}
		for(Block ba : ball){
			ba.x+=ba.dx;
			...
			for(Block b : blocks){
				if((b.left.intersects(ba) || b.right.intersects(ba)) && !b.destroyed){
					...
					b.destroyed = true;
					if(b.powerup)
						powerup.add(new Block(b.x, b.y, 30, 30, "extra.png"));
				}
